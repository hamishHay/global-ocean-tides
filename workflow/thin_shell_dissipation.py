...
def membraneUpsBeta(shell_thickness, ocean_thickness, freq, mass, radius, grav_surf, den_ocean, den_shell, n_max=12):
    ...
    # rigid_shell = (Te*rigid_shell[0] + Tv*rigid_shell[1])/shell_thickness
    ...
    pois_ratio = 1./np.sum( (mu_v / (1 - p_v)) * T ) * np.sum( p_v*(mu_v / (1 - p_v)) * T )
...
for q in q_nij:
    ...
    beta, ups, spring = membraneUpsBeta(h_shell, 50e3, abs(q), mass, radius, grav, den, den, n_max=n_max)
    solver.set_beta(beta)
...
ax.semilogy(ho/1e3, (np.sum(E22+E20+E21, axis=0)), '-', lw=1.5)
ax.semilogy(ho/1e3, (np.sum(Ecrust22+Ecrust20+Ecrust21, axis=0)), '--', lw=1.0)

ax.set_ylim([0.1, 1e13])
