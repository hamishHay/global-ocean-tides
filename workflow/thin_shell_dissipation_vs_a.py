...
coeff_file = h5py.File("europa_io_a_coeffs.h5", 'r')
...
E22 = np.zeros(( len(q_nij[0]), len(semimajor_axes)))
E20 = np.zeros(( len(q_nij[0]), len(semimajor_axes)))
E21 = np.zeros(( len(q_nij[0]), len(semimajor_axes)))

Ecrust22 = np.zeros(( len(q_nij[0]), len(semimajor_axes) ))
Ecrust21 = np.zeros(( len(q_nij[0]), len(semimajor_axes) ))
Ecrust20 = np.zeros(( len(q_nij[0]), len(semimajor_axes) ))

DISP = np.zeros(( len(q_nij[0]), len(semimajor_axes) ))
...
for i in range(len(semimajor_axes)):
    ...
    print("Solving a="+str(i/len(semimajor_axes)), end="\r", flush=True)
