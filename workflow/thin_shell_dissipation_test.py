...
def membraneUpsBeta(shell_thickness, ocean_thickness, freq, mass, radius, grav_surf, den_ocean, den_shell, visc_crit, n_max=12):
    ...
    def mu_andrade(eta, rigid, freq):
        J = 1.0/rigid
        a = 0.2
        af = math.gamma(a + 1) # get factorial of non-integer
        C = af * np.sin(a*np.pi*0.5)
        S = af * np.cos(a*np.pi*0.5)
        chi = 1.0



        N1 = J * eta**2.0 * freq**2.0 * ( C *(J*eta*freq*chi)**(-a) + 1)
        N2 = eta*freq * ((J*eta*freq)**(1-a)*chi**(-a)*S + 1)
        D = 1 + (J*eta*freq)**2.0
        D += (J*eta*freq)**(2*(1-a)) * chi**(-2*a) * af**2.0
        D += 2*(J*eta*freq)**(2-a) * chi**(-a) * (S/(J*eta*freq) + C)
    ...
    # mu_v = mu_andrade(visc_shell, rigid_shell, freq)
...
parser.add_argument('--density', type=float, default=1e3,
                    help='fluid density')
parser.add_argument('--visc', type=float, default=1e15,
                    help='shell viscosity')
...
if save_name is None:
    save_name = "LTE_solutions_alpha_" + str(alpha) + "_visc_" + str(int(np.log10(visc)))
...
if save:
    save_file = h5py.File(save_name + ".h5",'a')
...
beta, ups, spring = membraneUpsBeta(h_shell, 10e3, 10*rot_rate, mass, radius, grav, den, den, visc, n_max=n_max)
...
SPRINGS = np.zeros( ( len(freqs),  8), dtype=np.complex)
betas = np.zeros( (len(freqs), n_max), dtype=np.complex)
count = 0
for q in q_nij:

    beta, ups, spring = membraneUpsBeta(h_shell, 50e3, abs(q), mass, radius, grav, den, den, visc, n_max=n_max)
...
PHI = np.zeros( ( len(ho), len(freqs), 3, 8 ), dtype=np.complex )
PSI = np.zeros( ( len(ho), len(freqs), 3, 8 ), dtype=np.complex )

# print(q_nij, rot_rate)
...
for i in range(len(ho)):
    ...
    for j in range(len(freqs)):
        ...
        # beta, ups, spring = membraneUpsBeta(h_shell, 50e3, abs(forcing_freq), mass, radius, grav, den, den, visc, n_max=n_max)
        # # print(h_shell, 50e3, abs(forcing_freq), mass, radius, grav, den, den) visc,
        
        # beta[0] = 0.0000001
        # print(beta[1])
        
        solver.set_beta(betas[j])
        # print(forcing_freq, rot_rate)

        # --------------------------------------------------------------
        # Solve for libration component, P22
        if abs(a_22c + b_22s) > 1e-14:
            ...
            # print(freqs[j], psi, phi)

            if save:
                PHI[i, j, 2, :] = phi[:8]
                PSI[i, j, 2, :] = psi[:8]

                # print(phi[1])#, beta[1], solver.lambs, solver.rot_force)
                # print(solver.LTE_mat[3,3], solver.Kn, alpha/(2*rot_rate))
...
# print(np.sum(E22+E20+E21, axis=0))
